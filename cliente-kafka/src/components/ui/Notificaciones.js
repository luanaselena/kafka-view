...
const Notificaciones = () => {
	...
	const fetchApi = async () => {
		...
		const followers = await apiAxios.get("http://localhost:8080/getFollowers", {
			params: { username: usernameSesion },
		});
		...
		<div className={classes.root}>
			<Container maxWidth="md">
				<Grid container spacing={3}>
					<Grid item xs={12}>
						<h1 className={classes.paper}>NOTIFICACIONES</h1>
					</Grid>
					<Grid item xs={6}>
					...
					<Grid item xs={6}>
						{likes === undefined || likes.length === 0 ? (
							...
							likes.map((like) => (
								<div>
									{like.userLike.username} te dio like en el post "
									{like.post.title}"
								</div>
							))
						)}
